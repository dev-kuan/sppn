            ...
            <div class="px-6 py-4">
                <nav aria-label="Progress">
                    <ol role="list" class="flex items-center">
                        <li class="relative pr-8 sm:pr-20" :class="currentStep >= 1 ? 'flex-1' : ''">
                            <div class="absolute inset-0 flex items-center" aria-hidden="true">
                                <div class="h-0.5 w-full" :class="currentStep > 1 ? 'bg-indigo-600' : 'bg-gray-200'"></div>
                            </div>
                            <a @click="currentStep = 1" class="relative flex items-center justify-center w-8 h-8 rounded-full cursor-pointer group"
                               :class="currentStep >= 1 ? 'bg-indigo-600' : 'bg-white border-2 border-gray-300'">
                                <span class="h-2.5 w-2.5 rounded-full" :class="currentStep >= 1 ? 'bg-white' : 'bg-gray-300'"></span>
                                <span class="sr-only">Step 1</span>
                            </a>
                            <div class="mt-2 text-xs font-medium text-center" :class="currentStep >= 1 ? 'text-indigo-600' : 'text-gray-500'">
                                Observasi Harian
                            </div>
                        </li>

                        <li class="relative flex-1">
                            <a @click="goToReviewStep()" class="relative flex items-center justify-center w-8 h-8 rounded-full cursor-pointer group"
                               :class="currentStep >= 2 ? 'bg-indigo-600' : 'bg-white border-2 border-gray-300'">
                                <span class="h-2.5 w-2.5 rounded-full" :class="currentStep >= 2 ? 'bg-white' : 'bg-gray-300'"></span>
                                <span class="sr-only">Step 2</span>
                            </a>
                            <div class="mt-2 text-xs font-medium text-center" :class="currentStep >= 2 ? 'text-indigo-600' : 'text-gray-500'">
                                Review & Catatan
                            </div>
                        </li>
                    </ol>
            ...
            <div class="relative px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="fixed z-50 flex items-center space-x-3 bottom-10 right-5">
                        ...
                        <!-- Next/Submit Button -->
                        <button x-show="currentStep === 1" @click="goToReviewStep()"
                        ...
                        <button x-show="currentStep === 2" @click="submitAssessment()"
        ...
        <div x-show="currentStep === 1" x-transition>
            ...
            <div class="sticky z-30 mb-4 bg-white border-b border-gray-200 shadow-sm top-16">
                <div class="flex overflow-x-auto">
                    ...
                    <div class="flex">
                        @for($day = 1; $day <= $assessment->tanggal_penilaian->daysInMonth(); $day++)
                            ...
                            <div class="flex hover:bg-gray-50">
                                <div class="flex-shrink-0 w-64 px-4 py-3 text-sm text-gray-700 border-r border-gray-200">
                                    ...
                                    <div class="mt-1 text-xs text-gray-500">
                                        @php
                $itemData = collect($observationItemsArray)->firstWhere('id', $item->id);
    @endphp

    Bobot: {{ $item->bobot }} |
    Frekuensi: {{ $itemData['frekuensi'] ?? 0 }} |
                                        Terisi:
                                        <span class="font-semibold text-indigo-600"
                                            x-text="checkedCounts[{{ $item->id }}] ?? 0"></span>
                                        / {{ $itemData['frekuensi'] ?? 0 }}
                                    </div>
                                </div>
        ...
        <!-- STEP 2: Aspect Score Review -->
        <div x-show="currentStep === 2" x-transition>
            <div class="mb-6 bg-white rounded-lg shadow">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">Review Skor Per Aspek</h3>
                    <p class="mt-1 text-sm text-gray-500">Berikan catatan untuk setiap aspek berdasarkan hasil penilaian</p>
                </div>
                            ...
                            <div class="px-6 py-6">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        ...
                                        <p class="text-xs text-gray-500">{{ $variabel->nama }}</p>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-2xl font-bold text-indigo-600">
                                            {{ $aspectScore ? number_format($aspectScore->skor, 2) : '0.00' }}
                                        </div>
                                        <div class="px-3 py-1 mt-1 text-xs font-medium rounded-full inline-flex
                                ...
                                <div>
                                    <label class="block mb-2 text-sm font-medium text-gray-700">
                                        Catatan Skor <span class="text-red-500">*</span>
                                    </label>
                                    <textarea
                                        x-model="aspectNotes[{{ $aspek->id }}]"
                                        @input="saveAspectNote({{ $aspek->id }})"
                                        rows="3"
                                        placeholder="Berikan catatan atau komentar tentang hasil penilaian aspek ini..."
                                        class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                        required>{{ $aspectScore->catatan ?? '' }}</textarea>
                                </div>
                            </div>
                        @endforeach
                    @endforeach
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-blue-700">
                            <span class="font-medium">Pastikan semua catatan telah diisi</span> sebelum melakukan submit.
                            Catatan akan digunakan untuk evaluasi dan rekomendasi lebih lanjut.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-700">
                    Catatan Rekomendasi <span class="text-red-500">*</span>
                </label>
                <textarea x-model="[{{ $assessment->id }}]" @input="saveRecomendation({{ $assessment->id }})" rows="3"
                    placeholder="Berikan catatan atau komentar yang menjadi rekomendasi  ..."
                    class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    required>{{ $assessment->commitmentRecommendations->deskripsi ?? '' }}</textarea>
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
    ...
    function assessmentForm() {
        return {
            ...
            currentStep: 1,
            aspectNotes: @json($assessment->assessmentScores->pluck('catatan', 'aspect_id')),
            recomendation: @json([
    $assessment->commitmentRecommendation?->layak_dapat_hak =>
    $assessment->commitmentRecommendation?->deskripsi
]),

            observationItems: @json($observationItemsArray),

            scores: {
                kepribadian: Number({{ $assessment->skor_kepribadian ?? 0 }}),
                kemandirian: Number({{ $assessment->skor_kemandirian ?? 0 }}),
                sikap: Number({{ $assessment->skor_sikap ?? 0 }}),
                mental: Number({{ $assessment->skor_mental ?? 0 }}),
                total: Number({{ $assessment->skor_total ?? 0 }})
            },
            ...
            init() {
                ...
                this.autoSaver = new AutoSave({
                    ...
                    onSuccess: (result) => {
                        if (result.scores) {
                            this.scores = {
                                kepribadian: Number(result.scores.kepribadian) || 0,
                                kemandirian: Number(result.scores.kemandirian) || 0,
                                sikap: Number(result.scores.sikap) || 0,
                                mental: Number(result.scores.mental) || 0,
                                total: Number(result.scores.total) || 0,
                            };
                        }
            ...
            async saveAspectNote(aspectId) {
                const note = this.aspectNotes[aspectId];
                ...
                try {
                    ...
                    const response = await fetch('{{ route("assessments.update-aspect-note", $assessment) }}', {
                        method: 'POST',
                        headers: {
                    ...
                    console.error('Error saving aspect note:', error);
                    this.showErrorNotification('Gagal menyimpan catatan aspek');
                } finally {
                    this.saving = false;
                }
            },
            async saveRecomendation(assesmentId) {
                const note = this.saveRecomendation[assesmentId];
                ...
                try {
                    ...
                    const response = await fetch('{{ route("assessments.update-recomendation", $assessment) }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    ...
                    if (response.ok && result.success) {
                        ...
                        throw new Error(result.message || 'Gagal menyimpan catatan rekomendasi');
                    }
                } catch (error) {
                    console.error('Error saving recomendation:', error);
                    this.showErrorNotification('Gagal menyimpan catatan rekomendasi');
                } finally {
                    this.saving = false;
                }
            },
            ...
            async submitAssessment() {
                // Check all notes are filled
                const allNotesFilled = Object.values(this.aspectNotes).every(note => note && note.trim() !== '');

                if (!allNotesFilled) {
                    alert('Mohon isi semua catatan aspek sebelum submit.');
                    return;
                }
